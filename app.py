...
# --- 🛡️ 2. HIGH-PRECISION DATA ENGINE ---
...
st.markdown(f"<p style='text-align:center; color:white; font-size:12px; margin-bottom:10px;'>CORE V105.0 | PROFIT TARGET: 20 PTS | LIVE SYNC ACTIVE</p>", unsafe_allow_html=True)

# --- 🛠️ 4. SIDEBAR (63-POINT CONTROLS) ---
with st.sidebar:
    st.header("⚙️ System Control")
    asset = st.selectbox("🎯 Target Market:", ["NIFTY 50", "BANK NIFTY", "GOLD", "CRUDE OIL"])
    trade_limit = st.slider("Daily Sniper Limit:", 1, 10, 3)
    st.write("---")
    st.info("Jarvis is tracking FII/DII flows & Global News...")
...
def render_dashboard(ticker, gap):
    ...
    if st.session_state.trade_count >= trade_limit:
        live_area.success(f"🎯 {trade_limit} TRADES DONE. Profit Locked. System Sleeping.")
        return

    df = fetch_data(ticker)
    if df is not None and len(df) > 20:
        # 🛰️ A. FII/DII & NEWS ENGINE
        ...
        else: sig_text, sig_color = "⌛ SCANNING FOR 20PT MOVE...", "#555555"
        ...
        if "MASTER" in sig_text and st.session_state.last_sig != sig_text:
            ...
            voice = f"Attention! Jarvis Alert. {sig_text}. Target 20 points confirmed."
            ...
            with c2:
                st.markdown(f"""
                    <div style="background:#111; padding:20px; border-radius:15px; border:1px solid #333; text-align:center; height:400px; display:flex; flex-direction:column; justify-content:center;">
                        ...
                        <p style="color:gray; margin:5px 0;">MOOD: {news_mood} | FII: {news_score}</p>
...
# --- 🛰️ 6. EXTRA POINTS (IPO & NEWS) ---
st.write("---")
f1, f2 = st.columns(2)
with f1: st.info("🌐 **IPOs:** Fractal Analytics (9 Jan) | **FII:** Aggressive Buying")
with f2: st.info("📊 **Global:** Nasdaq Bullish | **News:** Crude Oil inventories up")
